...
let%template run_test_for_all_producers_and_operations ~iter ~iter' ~create_consumer =
  ...
  let%bind () =
    (run_test_for_all_producers [@mode local])
      ...
      ~iter'
      ~create_consumer
      ~operation:(Filter_map (fun x -> exclave_ Some x))
  in
  let%bind () =
    (run_test_for_all_producers [@mode local])
      ~iter
      ~iter'
      ~create_consumer
      ~operation:(Concat_map (fun x -> exclave_ [ x ]))
  in
  return ()
;;
...
module%test [@name "stop conditions"] _ =
  [%template
...
module%test [@name "pushback"] _ = struct
  let%expect_test _ =
...
module%test [@name "staged-specific behavior"] _ = struct
...
module%test [@name "operation-specific semantics"] _ = struct
